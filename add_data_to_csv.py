...
existing_data_df = pd.read_csv('file_hex_data.csv')

# Convert the data to a DataFrame
new_data_df = pd.DataFrame(data_to_add)

# Concatenate existing data with new data
combined_df = pd.concat([existing_data_df, new_data_df])

# Remove duplicates based on the Filename and MagicBytes columns
combined_df = combined_df.drop_duplicates(subset=['Filename', 'MagicBytes'])

# Save the updated DataFrame back to the CSV
combined_df.to_csv('file_hex_data.csv', index=False)

print("Data added successfully, duplicates removed, and saved to the CSV file.")
